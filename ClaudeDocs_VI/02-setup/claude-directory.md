> Nguồn: https://code.claude.com/docs/en/claude-directory.md
> Tài liệu Claude Code — bản dịch tiếng Việt

# Khám phá thư mục .claude

> Nơi Claude Code đọc CLAUDE.md, settings.json, hook, skill, command, subagent, workflow, rule và bộ nhớ tự động (auto memory). Khám phá thư mục .claude trong dự án của bạn và ~/.claude trong thư mục home.
...
Claude Code đọc các chỉ dẫn, cấu hình, skill, subagent và bộ nhớ (memory) từ thư mục dự án của bạn và từ `~/.claude` trong thư mục home. Hãy commit các file dự án vào git để chia sẻ với đội của bạn; các file trong `~/.claude` là cấu hình cá nhân áp dụng cho toàn bộ dự án của bạn.

Trên Windows, `~/.claude` được phân giải thành `%USERPROFILE%\.claude`. Nếu bạn thiết lập [`CLAUDE_CONFIG_DIR`](/en/env-vars), mọi đường dẫn `~/.claude` trên trang này sẽ nằm dưới thư mục đó thay thế.

Hầu hết người dùng chỉ chỉnh sửa `CLAUDE.md` và `settings.json`. Phần còn lại của thư mục là tùy chọn: thêm skill, rule hoặc subagent khi bạn cần.

## Khám phá thư mục

Nhấp vào các file trong cây thư mục để xem mỗi file làm gì, khi nào nó được nạp, và một ví dụ.

<ClaudeExplorer />

## Những gì không được hiển thị

Trình khám phá bao gồm các file mà bạn tự tạo và chỉnh sửa. Một vài file liên quan nằm ở nơi khác:

| File                    | Vị trí                     | Mục đích                                                                                                                                                                                                                                                          |
| ----------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `managed-settings.json` | Cấp hệ thống, tùy theo OS  | Cấu hình do doanh nghiệp áp đặt mà bạn không thể ghi đè. Xem [server-managed settings](/en/server-managed-settings).                                                                                                                                                |
| `CLAUDE.local.md`       | Thư mục gốc dự án          | Tùy chọn riêng tư của bạn cho dự án này, được nạp cùng với CLAUDE.md. Tạo thủ công và thêm vào `.gitignore`.                                                                                                                                                        |
| Plugin đã cài đặt       | `~/.claude/plugins`        | Các marketplace đã clone, các phiên bản plugin đã cài, và dữ liệu riêng của từng plugin, được quản lý bởi các lệnh `claude plugin`. Các phiên bản mồ côi sẽ bị xóa 7 ngày sau khi cập nhật hoặc gỡ cài plugin. Xem [plugin caching](/en/plugins-reference#plugin-caching-and-file-resolution). |

`~/.claude` cũng chứa dữ liệu mà Claude Code ghi trong quá trình bạn làm việc: bản ghi hội thoại (transcript), lịch sử prompt, snapshot file, cache và log. Xem [application data](#application-data) bên dưới.

## Chọn đúng file

Các loại tùy chỉnh khác nhau nằm trong các file khác nhau. Dùng bảng này để tìm nơi một thay đổi thuộc về.

| Bạn muốn                                            | Chỉnh sửa                                | Phạm vi           | Tham khảo                                          |
| :------------------------------------------------- | :--------------------------------------- | :---------------- | :------------------------------------------------- |
| Cung cấp cho Claude ngữ cảnh và quy ước của dự án   | `CLAUDE.md`                              | dự án hoặc toàn cục | [Memory](/en/memory)                               |
| Cho phép hoặc chặn các lời gọi tool cụ thể          | `settings.json` `permissions` hoặc `hooks` | dự án hoặc toàn cục | [Permissions](/en/permissions), [Hooks](/en/hooks) |
| Chạy một script trước hoặc sau các lời gọi tool     | `settings.json` `hooks`                  | dự án hoặc toàn cục | [Hooks](/en/hooks)                                 |
| Thiết lập biến môi trường cho phiên làm việc        | `settings.json` `env`                    | dự án hoặc toàn cục | [Settings](/en/settings#available-settings)        |
| Giữ các ghi đè cá nhân ngoài git                    | `settings.local.json`                    | chỉ dự án         | [Settings scopes](/en/settings#settings-files)     |
| Thêm một prompt hoặc khả năng gọi bằng `/name`      | `skills/<name>/SKILL.md`                 | dự án hoặc toàn cục | [Skills](/en/skills)                               |
| Định nghĩa một subagent chuyên biệt với tool riêng  | `agents/*.md`                            | dự án hoặc toàn cục | [Subagents](/en/sub-agents)                        |
| Điều phối nhiều subagent từ một script              | `workflows/*.js`                         | dự án hoặc toàn cục | [Dynamic workflows](/en/workflows)                 |
| Kết nối các tool bên ngoài qua MCP                  | `.mcp.json`                              | chỉ dự án         | [MCP](/en/mcp)                                     |
| Thay đổi cách Claude định dạng phản hồi             | `output-styles/*.md`                     | dự án hoặc toàn cục | [Output styles](/en/output-styles)                 |

## Tham chiếu file

Bảng này liệt kê mọi file mà trình khám phá bao gồm. Các file phạm vi dự án nằm trong repo của bạn dưới `.claude/` (hoặc ở thư mục gốc đối với `CLAUDE.md`, `.mcp.json` và `.worktreeinclude`). Các file phạm vi toàn cục nằm trong `~/.claude/` và áp dụng cho tất cả dự án.

<Note>
  Một số thứ có thể ghi đè những gì bạn đặt trong các file này:

  * [Managed settings](/en/server-managed-settings) do tổ chức của bạn triển khai được ưu tiên hơn mọi thứ
  * Các cờ CLI như `--permission-mode` hoặc `--settings` ghi đè `settings.json` cho phiên đó
  * Một số biến môi trường được ưu tiên hơn cấu hình tương đương của chúng, nhưng điều này khác nhau: hãy kiểm tra [tham chiếu biến môi trường](/en/env-vars) cho từng cái

  Xem [settings precedence](/en/settings#settings-precedence) để biết thứ tự đầy đủ.
</Note>

Nhấp vào tên file để mở node đó trong trình khám phá phía trên.

| File                                                | Phạm vi            | Commit | Chức năng                                                                                                     | Tham khảo                                                       |
| --------------------------------------------------- | ------------------ | ------ | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| [`CLAUDE.md`](#ce-claude-md)                        | Dự án và toàn cục  | ✓      | Chỉ dẫn được nạp mỗi phiên                                                                                    | [Memory](/en/memory)                                            |
| [`rules/*.md`](#ce-rules)                           | Dự án và toàn cục  | ✓      | Chỉ dẫn theo chủ đề, tùy chọn giới hạn theo đường dẫn                                                         | [Rules](/en/memory#organize-rules-with-claude/rules/)           |
| [`settings.json`](#ce-settings-json)                | Dự án và toàn cục  | ✓      | Quyền, hook, biến môi trường, model mặc định                                                                  | [Settings](/en/settings)                                        |
| [`settings.local.json`](#ce-settings-local-json)    | Chỉ dự án          |        | Ghi đè cá nhân của bạn, được gitignore khi Claude Code tạo ra nó                                              | [Settings scopes](/en/settings#settings-files)                  |
| [`.mcp.json`](#ce-mcp-json)                         | Chỉ dự án          | ✓      | MCP server chia sẻ trong đội                                                                                  | [MCP scopes](/en/mcp#mcp-installation-scopes)                   |
| [`.worktreeinclude`](#ce-worktreeinclude)           | Chỉ dự án          | ✓      | Các file bị gitignore cần sao chép vào worktree mới                                                           | [Worktrees](/en/worktrees#copy-gitignored-files-into-worktrees) |
| [`skills/<name>/SKILL.md`](#ce-skills)              | Dự án và toàn cục  | ✓      | Prompt tái sử dụng, gọi bằng `/name` hoặc tự động gọi                                                         | [Skills](/en/skills)                                            |
| [`commands/*.md`](#ce-commands)                     | Dự án và toàn cục  | ✓      | Prompt đơn file; cùng cơ chế với skill                                                                        | [Skills](/en/skills)                                            |
| [`output-styles/*.md`](#ce-output-styles)           | Dự án và toàn cục  | ✓      | Các phần system-prompt tùy chỉnh                                                                              | [Output styles](/en/output-styles)                              |
| [`agents/*.md`](#ce-agents)                         | Dự án và toàn cục  | ✓      | Định nghĩa subagent với prompt và tool riêng                                                                  | [Subagents](/en/sub-agents)                                     |
| [`workflows/*.js`](#ce-workflows)                   | Dự án và toàn cục  | ✓      | Script workflow động do Claude viết và lưu từ `/workflows`; mỗi file trở thành một lệnh `/<name>`             | [Dynamic workflows](/en/workflows)                              |
| [`agent-memory/<name>/`](#ce-agent-memory)          | Dự án và toàn cục  | ✓      | Bộ nhớ bền vững cho subagent                                                                                  | [Persistent memory](/en/sub-agents#enable-persistent-memory)    |
| [`~/.claude.json`](#ce-claude-json)                 | Chỉ toàn cục       |        | Trạng thái ứng dụng, OAuth, các công tắc UI, MCP server cá nhân                                               | [Global config](/en/settings#global-config-settings)            |
| [`projects/<project>/memory/`](#ce-global-projects) | Chỉ toàn cục       |        | Auto memory: các ghi chú của Claude cho chính mình qua các phiên                                              | [Auto memory](/en/memory#auto-memory)                           |
| [`keybindings.json`](#ce-keybindings)               | Chỉ toàn cục       |        | Phím tắt bàn phím tùy chỉnh                                                                                   | [Keybindings](/en/keybindings)                                  |
| [`themes/*.json`](#ce-themes)                       | Chỉ toàn cục       |        | Chủ đề màu tùy chỉnh                                                                                          | [Custom themes](/en/terminal-config#create-a-custom-theme)      |

## Khắc phục sự cố cấu hình

Nếu một cấu hình, hook hoặc file không có hiệu lực, hãy xem [Debug your configuration](/en/debug-your-config) để biết các lệnh kiểm tra và bảng tra cứu theo triệu chứng.

## Dữ liệu ứng dụng

Ngoài cấu hình bạn tự tạo, `~/.claude` còn chứa dữ liệu mà Claude Code ghi trong các phiên làm việc. Các file này là văn bản thuần (plaintext). Bất cứ thứ gì đi qua một tool đều nằm trong một transcript trên đĩa: nội dung file, output lệnh, văn bản dán.

### Dọn dẹp tự động

Các file trong các đường dẫn bên dưới sẽ bị xóa khi khởi động một khi chúng cũ hơn [`cleanupPeriodDays`](/en/settings#available-settings). Mặc định là 30 ngày.

| Đường dẫn dưới `~/.claude/`                  | Nội dung                                                                                                                  |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `projects/<project>/<session>.jsonl`         | Bản ghi hội thoại đầy đủ: mọi tin nhắn, lời gọi tool và kết quả tool                                                     |
| `projects/<project>/<session>/subagents/`    | Bản ghi hội thoại của [subagent](/en/sub-agents), bị xóa cùng transcript của phiên cha khi nó hết hạn                     |
| `projects/<project>/<session>/tool-results/` | Các output tool lớn được tràn ra các file riêng                                                                          |
| `file-history/<session>/`                    | Snapshot trước khi chỉnh sửa của các file Claude đã thay đổi, dùng cho [checkpoint restore](/en/checkpointing)           |
| `plans/`                                     | Các file kế hoạch được viết trong [plan mode](/en/permission-modes#analyze-before-you-edit-with-plan-mode)               |
| `debug/`                                     | Log debug theo từng phiên, chỉ được ghi khi bạn khởi động với `--debug` hoặc chạy `/debug`                               |
| `paste-cache/`, `image-cache/`               | Nội dung của các đoạn dán lớn và ảnh đính kèm                                                                            |
| `session-env/`                               | Metadata môi trường theo từng phiên                                                                                      |
| `tasks/`                                     | Danh sách tác vụ theo từng phiên được các tool tác vụ ghi                                                                |
| `shell-snapshots/`                           | Môi trường shell được ghi lại dùng bởi tool Bash. Bị xóa khi thoát sạch. Lượt quét dọn sẽ xóa những gì còn sót sau khi crash. |
| `backups/`                                   | Bản sao có dấu thời gian của `~/.claude.json` được lấy trước các lần di trú cấu hình                                     |
| `feedback-bundles/`                          | Kho lưu trữ transcript đã ẩn danh do `/feedback` ghi trên các nhà cung cấp bên thứ ba, để gửi cho nhóm phụ trách tài khoản Anthropic của bạn |
| `todos/`, `statsig/`, `logs/`                | Các thư mục cũ từ phiên bản trước. Không còn được ghi nữa. Lượt quét dọn xóa nội dung của chúng rồi xóa thư mục rỗng.    |

### Giữ lại cho tới khi bạn xóa

Các đường dẫn sau không được dọn dẹp tự động và tồn tại vô thời hạn.

| Đường dẫn dưới `~/.claude/` | Nội dung                                                                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `history.jsonl`         | Mọi prompt bạn đã gõ, kèm dấu thời gian và đường dẫn dự án. Dùng để gọi lại bằng phím mũi tên lên.                                                                              |
| `stats-cache.json`      | Số liệu tổng hợp về token và chi phí hiển thị bởi `/usage`                                                                                                                     |
| `remote-settings.json`  | Bản sao cache của [server-managed settings](/en/server-managed-settings) cho tổ chức của bạn. Chỉ hiện diện khi tổ chức của bạn đã cấu hình chúng. Được làm mới mỗi lần khởi động. |

Các file cache và lock nhỏ khác xuất hiện tùy theo bạn dùng tính năng nào và có thể xóa an toàn.

### Lưu trữ dạng văn bản thuần

Transcript và lịch sử không được mã hóa khi lưu trữ. Quyền file của hệ điều hành là biện pháp bảo vệ duy nhất. Nếu một tool đọc file `.env` hoặc một lệnh in ra thông tin xác thực, giá trị đó sẽ được ghi vào `projects/<project>/<session>.jsonl`. Để giảm mức phơi nhiễm:

* Giảm `cleanupPeriodDays` để rút ngắn thời gian giữ transcript
* Thiết lập biến môi trường [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](/en/env-vars) để bỏ qua việc ghi transcript và lịch sử prompt ở mọi chế độ. Trong chế độ không tương tác, thay vào đó bạn có thể truyền `--no-session-persistence` cùng với `-p`, hoặc đặt `persistSession: false` trong Agent SDK.
* Dùng [permission rules](/en/permissions) để từ chối việc đọc các file thông tin xác thực

### Xóa dữ liệu cục bộ

Chạy `claude project purge` để xóa trạng thái mà Claude Code giữ cho một dự án. Lệnh này yêu cầu Claude Code v2.1.124 trở lên. Nó xóa:

* Transcript và auto memory dưới `projects/`
* Các mục `tasks/`, `debug/` và `file-history/` theo từng phiên
* Các dòng prompt khớp trong `history.jsonl`
* Mục của dự án trong `~/.claude.json`

Lệnh in ra toàn bộ kế hoạch xóa và yêu cầu xác nhận trước khi xóa bất cứ thứ gì.

Xem trước kế hoạch mà không xóa gì cả:

```bash theme={null}
claude project purge ~/work/my-repo --dry-run
```

Xóa với một lần nhắc xác nhận duy nhất:

```bash theme={null}
claude project purge ~/work/my-repo
```

Bỏ qua đường dẫn để chọn một dự án từ danh sách tương tác.

Bỏ qua lời nhắc xác nhận để dùng trong script:

```bash theme={null}
claude project purge ~/work/my-repo --yes
```

Truyền `--all` thay cho một đường dẫn để dọn trạng thái cho mọi dự án cùng lúc, điều này xóa hẳn `history.jsonl` thay vì lọc nó. Truyền `-i` để duyệt qua kế hoạch xóa từng mục một.

Lệnh này để nguyên `shell-snapshots/` và `backups/` vì chúng không thuộc phạm vi dự án, và cảnh báo về chúng trong output kế hoạch. Nó thoát với trạng thái 1 nếu không có trạng thái nào khớp với đường dẫn đã cho.

Bạn cũng có thể xóa bất kỳ đường dẫn dữ liệu ứng dụng nào ở trên bằng tay. Các phiên mới không bị ảnh hưởng. Bảng dưới đây cho thấy bạn mất gì đối với các phiên đã qua.

| Xóa                                                                                                                                                                                          | Bạn mất                                                       |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `~/.claude/projects/`                                                                                                                                                                        | Resume, continue và rewind cho các phiên đã qua              |
| `~/.claude/history.jsonl`                                                                                                                                                                    | Gọi lại prompt bằng phím mũi tên lên                         |
| `~/.claude/file-history/`                                                                                                                                                                    | Checkpoint restore cho các phiên đã qua                      |
| `~/.claude/stats-cache.json`                                                                                                                                                                 | Số liệu tổng lịch sử hiển thị bởi `/usage`                   |
| `~/.claude/remote-settings.json`                                                                                                                                                             | Không mất gì. Được lấy lại vào lần khởi động kế tiếp.        |
| `~/.claude/debug/`, `~/.claude/plans/`, `~/.claude/paste-cache/`, `~/.claude/image-cache/`, `~/.claude/session-env/`, `~/.claude/tasks/`, `~/.claude/shell-snapshots/`, `~/.claude/backups/` | Không mất gì mà người dùng thấy                              |
| `~/.claude/todos/`, `~/.claude/statsig/`, `~/.claude/logs/`                                                                                                                                  | Không mất gì. Thư mục cũ không được các phiên bản hiện tại ghi. |

Đừng xóa `~/.claude.json`, `~/.claude/settings.json` hoặc `~/.claude/plugins/`: chúng chứa thông tin xác thực, tùy chọn và plugin đã cài của bạn.

## Tài nguyên liên quan

* [Manage Claude's memory](/en/memory): viết và tổ chức CLAUDE.md, rule và auto memory
* [Configure settings](/en/settings): thiết lập quyền, hook, biến môi trường và model mặc định
* [Create skills](/en/skills): xây dựng các prompt và workflow tái sử dụng
* [Configure subagents](/en/sub-agents): định nghĩa các agent chuyên biệt với ngữ cảnh riêng
